...
def animar_boton(boton, cargando_flag, ventana, texto_final="Enviar"):
    def actualizar_texto():
        if not cargando_flag[0] or not boton.winfo_exists():
            boton.config(text=texto_final, state=tk.NORMAL)  # Texto final cuando se detiene
            return

        # Contar puntos para simular animación
        puntos = boton.cget("text").count(".") % 3 + 1
        boton.config(text="Cargando" + "." * puntos, state=tk.DISABLED)
...
class ModificarProductoForm:
    def __init__(self, parent, nom_libro, callback):
        ...
        self.ruta_imagen_local = None  # Inicializar ruta de imagen local
        ...
        w, h = 700, 500
        self.centrar_ventana(self.ventana_modificar, w, h)
    ...
    def crear_formulario_modificar_producto(self):
        ...
        button_cargar_imagen = ttk.Button(
            main_frame, 
            text="Cargar Imagen", 
            style="Custom.TButton", 
            command=lambda: self.cargar_imagen_modificar_producto(label_img)
        )
        button_cargar_imagen.grid(row=6, column=0, padx=10, pady=10, sticky='w')

        # Crear campos del formulario
        self.entry_nombre = self.crear_campo_formulario(main_frame, "Nombre del Libro:", producto[0], 30, 0)
        self.entry_precio = self.crear_campo_formulario(main_frame, "Precio:", producto[1], 30, 2)
        self.entry_stock = self.crear_campo_formulario(main_frame, "Stock:", producto[2], 30, 4)

        autores = self.obtener_autores()
        self.combo_autor = self.crear_combobox_formulario(main_frame, "Autor:", autores, producto[4], 28, 6)

        generos = self.obtener_generos()
        self.combo_genero = self.crear_combobox_formulario(main_frame, "Género:", generos, producto[5], 28, 8)
        ...
        self.button_modificar.pack(side=tk.LEFT, padx=10)
        ...
        button_cancelar.pack(side=tk.LEFT, padx=10)
    ...
    def cargar_imagen_modificar_producto(self, label_img):
        ...
        if file_path:
            ...
            try:
                ...
                messagebox.showerror("Error", f"No se pudo cargar la imagen seleccionada: {e}")

    def autenticar_google_drive(self):
        try:
            gauth = GoogleAuth()
            # Asegúrate de tener el archivo 'client_secrets.json' en el directorio adecuado
            ...
            messagebox.showerror("Error", f"Error al autenticar con Google Drive: {e}")
            return None
    ...
    def iniciar_modificacion(self):
        ...
        animar_boton(self.button_modificar, self.cargando, self.ventana_modificar, texto_final="Modificar")

        # Iniciar la modificación en un hilo separado para no bloquear la interfaz
        threading.Thread(target=self.modificar_producto_en_bd, daemon=True).start()

    def validar_formulario(self):
        nombre = self.entry_nombre.get().strip()
        precio = self.entry_precio.get().strip()
        stock = self.entry_stock.get().strip()
        autor = self.combo_autor.get().strip()
        genero = self.combo_genero.get().strip()

        if not (nombre and precio and stock and autor and genero):
        ...
        if not stock.isdigit():
            messagebox.showerror("Error", "El stock debe ser un valor numérico.")
            return False
        ...
        try:
            ...
            if self.ruta_imagen_local:
                ...
                archivo_drive = drive.CreateFile({
                    'parents': [{'id': '1nHP5tyVyjDPpXE7N5Tm4nAK63PhzUIEP'}],  # Reemplaza con tu carpeta de Google Drive
                    'title': os.path.basename(self.ruta_imagen_local)
                })
                archivo_drive.SetContentFile(self.ruta_imagen_local)
                archivo_drive.Upload()

                nueva_imagen_url = archivo_drive['alternateLink']
                print(f"Imagen subida correctamente: {nueva_imagen_url}")
            ...
            """
            params = [nombre, precio, stock, autor, genero]

            # Si hay una nueva imagen, agrega la columna y el valor
            if nueva_imagen_url:
                query += ", imagen=%s"
                params.append(nueva_imagen_url)

            query += " WHERE nom_libro=%s"
            params.append(self.nom_libro)

            cursor.execute(query, tuple(params))
            conexion.commit()

            # Programar el mensaje de éxito y el cierre de la ventana en el hilo principal
            self.parent.after(0, self.on_modificacion_exitosa)

        except Exception as e:
            # Programar el mensaje de error en el hilo principal
            self.parent.after(0, lambda: messagebox.showerror("Error", f"No se pudo modificar el producto: {e}"))
        finally:
            # Programar la detención de la animación en el hilo principal
            self.parent.after(0, self.detener_animacion)

    def on_modificacion_exitosa(self):
        messagebox.showinfo("Éxito", "Producto modificado correctamente.")
        self.callback()  # Actualizar la lista de productos
        self.cerrar_ventana()

    def detener_animacion(self):
        self.cargando[0] = False
        if self.button_modificar.winfo_exists():
            animar_boton(self.button_modificar, self.cargando, self.ventana_modificar, texto_final="Modificar")
