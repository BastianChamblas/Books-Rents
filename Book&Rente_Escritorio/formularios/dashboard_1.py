...
# Lista global para almacenar los meses seleccionados
selected_months = []

def actualizar_grafico(canvas, figure, ax, meses_lista, mes_a_numero, selected_year, selected_autor, selected_genero, selected_months):
    """
    Actualiza el gráfico de ventas comparativas basado en los filtros seleccionados.
    """
    ax.clear()
    ...
    if selected_months:
        mes_numeros = [mes_a_numero[mes] for mes in selected_months]
        placeholders = ','.join(['%s'] * len(mes_numeros))
        where_clauses_common.append(f"dtc.mes IN ({placeholders})")
        params_common.extend(mes_numeros)
    ...
    try:
        ...
        print(f"Error: {err}")
        ax.text(0.5, 0.5, 'Error de conexión a la base de datos',
                horizontalalignment='center', verticalalignment='center',
                transform=ax.transAxes, fontsize=12, color='red')
        ax.set_xticks([])
        ax.set_yticks([])
...
def cargar_contenido_dashboard_1(frame):
    ...
    def limpiar_filtros():
        ...
        actualizar_grafico(canvas, figure, ax, meses_lista, mes_a_numero, 2024, autor_combobox.get(), genero_combobox.get(), selected_months)
        update_title()
        update_data()
    ...
    def update_data():
        ...
        # Determinar los meses seleccionados
        if mes_combobox.get() == "Todos":
            meses_seleccionados = selected_months
        else:
            meses_seleccionados = [mes_combobox.get()] if mes_combobox.get() in meses_lista else []

        # Convertir los nombres de los meses a números
        if meses_seleccionados:
            mes_numeros = [mes_a_numero[mes] for mes in meses_seleccionados]
        else:
            mes_numeros = []
        ...
        if meses_seleccionados:
            if len(mes_numeros) == 1:
                where_clauses.append("dtc.mes = %s")
                params.append(mes_numeros[0])
            else:
                placeholders = ','.join(['%s'] * len(mes_numeros))
                where_clauses.append(f"dtc.mes IN ({placeholders})")
                params.extend(mes_numeros)
        ...
        try:
            ...
            LIMIT 1;
            """
            cursor.execute(query_libro, tuple(params))
            ...
            LIMIT 1;
            """
            cursor.execute(query_autor, tuple(params))
            ...
            LIMIT 1;
            """
            cursor.execute(query_genero, tuple(params))
            ...
            if meses_seleccionados:
                if len(mes_numeros) == 1:
                    ...
                    params_common.append(mes_numeros[0])
                else:
                    placeholders = ','.join(['%s'] * len(mes_numeros))
                    where_clauses_common.append(f"dtc.mes IN ({placeholders})")
                    params_common.extend(mes_numeros)
            ...
            selected_year_int = int(selected_año)
            ...
            """
            cursor.execute(query_ventas_actual, [selected_year_int] + params_common)
            ...
            print(f"Error: {err}")
            libro_popular_value_label.config(text="Error")
            autor_popular_value_label.config(text="Error")
            genero_popular_value_label.config(text="Error")
            valor_ventas_actual_value_label.config(text="$0.00")
            valor_ventas_anterior_value_label.config(text="$0.00")
            diferencia_ventas_value_label.config(text="$0.00")
            porcentaje_value_label.config(text="0.00%")

    # Función para manejar los checkboxes y actualizar datos
    def toggle_mes(mes):
        ...
        # Al interactuar con checkboxes, establecer el combobox a "Todos"
        mes_combobox.set("Todos")
        actualizar_grafico(canvas, figure, ax, meses_lista, mes_a_numero, int(año_combobox.get()), autor_combobox.get(), genero_combobox.get(), selected_months)
        update_data()
    ...
    for mes in meses_lista:
        ...
        # Inicialmente, agregar todos los meses a selected_months
        selected_months.append(mes)
    ...
    def seleccionar_mes(event):
        ...
        if selected_mes == "Todos":
            # No cambiar selected_months, ya que se manejan mediante checkboxes
            pass
        else:
    ...
    def seleccionar_año(event):
        ...
        update_title()
        actualizar_grafico(canvas, figure, ax, meses_lista, mes_a_numero, selected_year, autor_combobox.get(), genero_combobox.get(), selected_months)
        update_data()
    ...
    # Función para actualizar los datos de los recuadros
    # (Ya está implementada como `update_data`)

    # Llamar a 'update_title', 'update_data' y 'actualizar_grafico' para establecer el título y datos iniciales
    update_title()
    update_data()
    actualizar_grafico(canvas, figure, ax, meses_lista, mes_a_numero, 2024, autor_combobox.get(), genero_combobox.get(), selected_months)
