...
class AgregarArriendosForm:
    def __init__(self, master, db_config):
        """
        Inicializa el formulario de agregar arriendos.
        ...
        self.crear_formulario_agregar_arriendos()
    ...
    def iniciar_envio(self):
        ...
        threading.Thread(target=self.enviar_formulario_agregar_arriendos).start()
    ...
    def validar_formulario(self):
        ...
        if not (nombre_libro and stock and nombre_autor and nombre_genero):
        ...
        # Verificar si el stock es un valor numérico
        ...
        if not hasattr(self, 'ruta_imagen_local'):
            messagebox.showerror("Error", "Por favor, cargue una imagen para el arriendo.")
            return False

        return True

    def crear_formulario_agregar_arriendos(self):
        ...
        # Etiquetas y entradas para Nombre del Libro y Stock
        ...
        label_stock.grid(row=0, column=1, padx=5, pady=(40, 5), sticky='w')
        self.entry_stock = ttk.Entry(fields_frame, width=30, style="Custom.TEntry")
        self.entry_stock.grid(row=1, column=1, padx=5, pady=10, sticky='w')
    ...
    def enviar_formulario_agregar_arriendos(self):
        ...
        try:
            ...
            if autor_result:
                ...
                raise Exception(f"No se encontró el autor: {nombre_autor}")
            ...
            if genero_result:
                genero_id = genero_result[0]
            else:
                raise Exception(f"No se encontró el género: {nombre_genero}")

            # Subir la imagen a Google Drive
            drive = self.autenticar_google_drive()
            archivo_drive = drive.CreateFile({
                'parents': [{'id': '1nHP5tyVyjDPpXE7N5Tm4nAK63PhzUIEP'}],  # Reemplaza con tu ID de carpeta en Drive
                'title': nombre_libro
            })
            ...
            # Realizar la inserción en la tabla app_libroarr
            insert_query = """
                INSERT INTO app_libroarr (nom_libro, stock, imagen, id_autor_id, id_genero_id)
                VALUES (%s, %s, %s, %s, %s)
            """
            cursor.execute(insert_query, (nombre_libro, stock, self.ruta_imagen, autor_id, genero_id))
            conexion.commit()

            self.cargando_flag[0] = False
            messagebox.showinfo("Éxito", "Arriendo agregado correctamente")
            self.limpiar_formulario_agregar_arriendos()

        except Exception as e:
            self.cargando_flag[0] = False
            messagebox.showerror("Error", f"Error al agregar el arriendo: {e}")
        finally:
            if cursor:
                cursor.close()
            if conexion:
